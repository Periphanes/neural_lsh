...
def main():
    ...
    class BasDataset(torch.utils.data.Dataset):
        ...
        def  __getitem__(self, index):
            return torch.flatten(self.data_list[index][0]), self.data_list[index][1]
    
    alex_train_dataset = BasDataset(train_selected)
    alex_val_dataset = BasDataset(validation_selected)
    alex_test_dataset = BasDataset(test_selected)
